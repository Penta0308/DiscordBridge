...
import tk.skmserver.discordbridge.models.ChannelMinecraftMentionConfig;
...
public class TextUtil {
    ...
    public static Text formatForMinecraft(ChannelMinecraftConfigCore config, Message message) {
        ...
        Optional<User> authorOptional = message.getAuthor().asUser();

        if (!serverOptional.isPresent() || !authorOptional.isPresent()) return null;
        ...
        String nickname = (author.getNickname(server).isPresent()) ? author.getNickname(server).get() : author.getName();
        ...
        if (highestRole.isPresent()) {
            ...
            Optional<Color> roleColorOptional = highestRole.get().getColor();
            if (roleColorOptional.isPresent()) roleColor = roleColorOptional.get();
        }
        ...
        Optional<Activity> activityOptional = author.getActivity();
        if (activityOptional.isPresent()) s = s.replace("%g", activityOptional.get().getName());
        ...
        texts = formatUserMentions(texts, config.mention, message.getMentionedUsers(), server);
    ...
    private static List<Text> formatUserMentions(List<Text> texts, ChannelMinecraftMentionConfig config, List<User> mentions, Server server) {
        ...
        for (User mention : mentions) {
            ...
            String nick = (mention.getNickname(server).isPresent()) ? mention.getNickname(server).get() : mention.getName();
            ...
            if (role.isPresent()) {
                formatted = formatted.color(ColorUtil.getColor(role.get().getColor().orElse(Color.WHITE)));
            }
    ...
    private static List<Text> formatRoleMentions(List<Text> texts, ChannelMinecraftMentionConfig config, List<Role> mentions) {
        ...
        for (Role mention : mentions) {
            ...
            if (mention.getColor().isPresent()) {
                builder.color(ColorUtil.getColor(mention.getColor().get()));
            }
        ...
        for (Text text : texts) {
            ...
            while (matcher.find()) {
                ...
                if (!formattedMentions.containsKey(channelId)) {
                    Optional<Channel> channelOptional = DiscordBridge.getInstance().getBotClient().getChannelById(channelId);
                    if (!channelOptional.isPresent()) continue;
                    Channel channel = channelOptional.get();
                    Optional<ServerTextChannel> textChannelOptional = channel.asServerTextChannel();
                    if (!textChannelOptional.isPresent()) continue;
                    String channelName = textChannelOptional.get().getName();
                    String mentionText = ConfigUtil.get(config.channelTemplate, "#%s").replace("%s", channelName);
                    Text.Builder builder = Text.builder().append(TextSerializers.FORMATTING_CODE.deserialize(mentionText))
                            .onHover(TextActions.showText(Text.of("Mentioning channel " + channelName + ".")));
